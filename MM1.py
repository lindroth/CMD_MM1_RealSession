...
# Global Variables
CHANNEL = 4 # assume channel is constant for everything
class MM1(ControlSurface):
    def __init__(self, c_instance):
        ...
        with self.component_guard():
           # self._setup_transport_control()
            ...
            self.vumeter =CMDVUMeters(self, EncoderElement(MIDI_CC_TYPE, CHANNEL, 80, Live.MidiMap.MapMode.absolute),EncoderElement(MIDI_CC_TYPE, CHANNEL, 81, Live.MidiMap.MapMode.absolute))
    ...
    def mixer_control(self):
      ...
      #  mixer.master_strip().set_select_button(master_select_button)
        ...
        for track in range(4):
            gain_controls = []
            freq_control = []
            self.mixer.track_eq(track).set_gain_controls(tuple([EncoderElement(MIDI_CC_TYPE, CHANNEL, 14+track, Live.MidiMap.MapMode.absolute), EncoderElement(MIDI_CC_TYPE, CHANNEL, 10+track, Live.MidiMap.MapMode.absolute), EncoderElement(MIDI_CC_TYPE, CHANNEL, 6+track, Live.MidiMap.MapMode.absolute)]))
            self.mixer.track_eq(track).set_enabled(True)       
            self.mixer.track_filter(track).set_filter_controls(EncoderElement(MIDI_CC_TYPE, CHANNEL, 18+track, Live.MidiMap.MapMode.absolute), None)
            self.mixer.track_filter(track).set_enabled(True)             
            self.mixer.channel_strip(track).set_assign_buttons(SimpleButtonElement(False, MIDI_NOTE_TYPE, CHANNEL,19+count),SimpleButtonElement(False, MIDI_NOTE_TYPE, CHANNEL, 20+count))
            count += 4
    ...
    def session_control(self):
        ...
        self.scrollencoder = ScrollEncoderElement(MIDI_CC_TYPE, CHANNEL, 3, Live.MidiMap.MapMode.absolute, self.session)                   #assign a shift button so that we can switch states between the SessionComponent and the SessionZoomingComponent
